...
class OtpUserCubit extends Cubit<OtpUserState> {
  ...
  Future<void> validateOtpCode({required String phoneNumber, required String otpCode}) async {
  ...
  try {
    ...
    result.fold(
      (failure) {
        final errorMessage = failure.message.isEmpty ? 'Unknown error' : failure.message;
    ...
    emit(ValidateOtpCodeError('An unexpected error occurred: ${e.toString()}'));
  }
}
